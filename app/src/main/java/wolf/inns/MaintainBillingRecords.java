...
public class MaintainBillingRecords {
  ...
  public static double calculateRoomCharge(int stayId) {
    ...
    String sqlStatement =
        "SELECT ((DATEDIFF(checkoutDate, checkinDate)+1)*rate) AS \"Room Charge\" FROM stays NATURAL JOIN rooms WHERE stayId = ?";
   ...
   *
   * @return An ArrayList containing PaymentMethods objects for each of the returned records if
   ...
   *
   * @param paymentMethod the PaymentMethods object with new field values that needs to be updated
   *     in the database
  ...
  public static boolean updatePaymentMethod(PaymentMethods paymentMethod) {
    ...
    try {
      ...
      if (1 == rowsAffected) {
        ...
        System.out.println(
            "There is no existing payment method with payMethodCode = "
  ...
  public static BillingInfo createBillingInfo(
    ...
    String sqlStatement =
        "INSERT INTO billing_info (responsiblePartySSN, address, city, state, payMethodCode, cardNumber) VALUES (?, ?, ?, ?, ?, ?)";
    PreparedStatement statement = null;
  ...
  public static boolean updateBillingInfo(BillingInfo billingInfo) {
    ...
    String sqlStatement =
        "UPDATE billing_info SET responsiblePartySSN = ?, address = ?, city = ?, state = ?, payMethodCode = ?, cardNumber = ?, totalCharges = ? WHERE billingId = ?";
    Connection connection = null;
    PreparedStatement statement = null;
  ...
  /**
   * The sum of service record charges and room charge, if paid with hotel CC gives discounted
   * charge.
   *
  ...
  public static double generateBillingTotal(int stayId) throws SQLException {
    ...
    String SqlString2 =
        String.format(
            "SELECT payMethodCode FROM stays\n"
   ...
   */
  public static boolean updateBillingInfoTotalCharges(
      int billingId, double totalCharges, Connection connection) {
