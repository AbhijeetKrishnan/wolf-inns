...
public class ServiceRecords extends DatabaseObject {
  ...
  public static ServiceRecords select() {
    ArrayList<ServiceRecords> serviceRecordsList =
        MaintainingServiceRecords.retrieveAllServiceRecords();
    ArrayList<DatabaseObject> databaseObjectList =
        (ArrayList<DatabaseObject>) ((ArrayList<?>) serviceRecordsList);

    ServiceRecords selectedRecord =
        (ServiceRecords) MenuUtilities.paginatedRecordSelection(databaseObjectList);
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof ServiceRecords)) {
      return false;
    }
    ServiceRecords s = (ServiceRecords) o;
    return (stayId == s.stayId
        && serviceCode.equals(s.serviceCode)
        && staffId == s.staffId
        && serviceDate.equals(s.serviceDate)
  ...
  public String toString() {
    return String.format(
        "|%12d|%12s|%12d|%12s|%12s|", stayId, serviceCode, staffId, serviceDate, serviceTime);
  }
}
