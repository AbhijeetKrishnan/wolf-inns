...
public class MenuInformationProcessing {
  ...
  public static void menuOptionUpdateHotel() {
    ...
    if (null == h) {
      ...
      String[] current = {
        ...
        h.getState(),
        h.getPhone(),
        Integer.toString(h.getManagerId())
      };
      String[] update = {
        h.getName(),
        h.getAddress(),
  ...
  public static void menuOptionCreateRoom() {
    try {
      ...
      Rooms r =
          InformationProcessing.createRoom(
              hotelId, roomNumber, maxAllowedOcc, rate, categoryCode, available);
    } catch (RuntimeException ex) {
      ex.printStackTrace();
    }
  ...
  public static void menuOptionCreateCustomer() {
    ...
    Customers c =
        InformationProcessing.createCustomer(response[0], response[1], response[2], response[3]);
    if (null == c) {
      System.out.println("Something unexpected happened while attempting to create the record.");
    } else {
      System.out.println("Record created successfully.");
  ...
  public static void menuOptionDeleteCustomer() {
    ...
    if (null == c) {
      ...
      Scanner scanner = new Scanner(System.in);
      System.out.println(
          "Attempting to delete customer record with customer id: " + c.getCustomerId());
      System.out.println("Are you sure you want to delete this record? (Y/N)");
      String response = "";
      do {
        System.out.print("===> ");
  ...
  public static void menuOptionDeleteRoomCategory() {
    ...
    if (null == r) {
      ...
      Scanner scanner = new Scanner(System.in);
      System.out.println(
          "Attempting to delete room category record with room category code: "
              + r.getCategoryCode());
      System.out.println("Are you sure you want to delete this record? (Y/N)");
      String response = "";
      do {
  ...
  // Accepts nothing and returns nothing. Fully encapsulated dialog for service
  // staff entry!
  public static void menuOptionCreateServiceStaff() {
    ...
    if (null != serviceStaffList && serviceStaffList.size() > 0) {
      ...
      ArrayList<DatabaseObject> databaseObjectList =
          (ArrayList<DatabaseObject>) ((ArrayList<?>) serviceStaffList);
      ...
      if (null == staff) {
        ...
        if (null == hotel) {
          ...
          // Create the service staff record using the required values from the
          ...
          if (null == serviceStaff) {
            System.out.println(
                "Something unexpected happened while attempting to create the record.");
          } else {
            System.out.println("Record created successfully.");
          }
        }
  ...
  public static void menuOptionUpdateServiceStaff() {
    ...
    if (null == serviceStaff) {
      ...
      // For every update eligible field, ask the user if they want to update it, if
      // yes then accept
      // new input and set value in object
  ...
  public static void menuOptionDeleteServiceStaff() {
    ...
    if (null == serviceStaff) {
      ...
      // Update hotelId field
      System.out.println(
          "Attempting to delete service staff assignment record with staff Id: "
  ...
  public static void menuOptionCreateRoomCategoriesServices() {
    ...
    ArrayList<RoomCategories> roomCategoriesList =
        InformationProcessing.retrieveAllRoomCategories();

    if (null != roomCategoriesList && roomCategoriesList.size() > 0) {

      ArrayList<DatabaseObject> databaseObjectList =
          (ArrayList<DatabaseObject>) ((ArrayList<?>) roomCategoriesList);

      // Have the user select a room category record to work with
      RoomCategories roomCategory =
          (RoomCategories) MenuUtilities.paginatedRecordSelection(databaseObjectList);

      if (null == roomCategory) {
        System.out.println("No record was selected.");
      } else {
        System.out.println(
            "Choose a service to assign to room category " + roomCategory.getCategoryCode() + ".");
        ...
        if (null == service) {
          ...
          if (InformationProcessing.assignServiceToRoomCategory(
              roomCategory.getCategoryCode(), service.getServiceCode())) {
            System.out.println("Record created successfully.");
          } else {
            System.out.println(
                "Something unexpected happened while attempting to create the record.");
  ...
  public static void menuOptionDeleteRoomCateogoriesServices() {
    ...
    if (null == roomCategoriesServices) {
      ...
      // Update hotelId field
      System.out.println(
          "Attempting to delete room category service assignment record with room category "
  ...
  public static void menuOptionCheckin() {
    ...
    ArrayList<Rooms> roomsList =
        HotelStayOperations.retrieveAvailableRooms(
            hotel.getHotelId(), roomCategory.getCategoryCode());
    ArrayList<DatabaseObject> databaseObjectList =
        (ArrayList<DatabaseObject>) ((ArrayList<?>) roomsList);
    ...
    if (null != room) {
      ...
      System.out.println(
          "Enter the street address of the person responsible for payment (75 characters max).");
      ...
      System.out.println(
          "Enter the city of the person responsible for payment (50 characters max).");
      ...
      System.out.println(
          "Enter the state of the person responsible for payment (2 character state).");
      ...
      String cardNumber = "";
      if (paymentMethod.getPayMethodCode().equals("CCWF")
          || paymentMethod.getPayMethodCode().equals("CCON")) {
      ...
      boolean checkinResult =
          HotelStayOperations.checkinStay(
              room.getHotelId(),
              room.getRoomNumber(),
              customer.getCustomerId(),
              numOfGuests,
              servicingStaff.getStaffId(),
              responsiblePartySSN,
              address,
              city,
              state,
  ...
  public static void menuOptionCheckout() {
    ...
    ArrayList<DatabaseObject> databaseObjectList =
        (ArrayList<DatabaseObject>) ((ArrayList<?>) staysList);
  ...
  public static void menuOptionSearchAvailableRooms() {
    ...
    ArrayList<Rooms> availableRooms =
        HotelStayOperations.retrieveAvailableRooms(
            hotel.getHotelId(), roomCategory.getCategoryCode());
    ...
    if (null == availableRooms || availableRooms.size() == 0) {
      ...
      ArrayList<DatabaseObject> databaseObjectList =
          (ArrayList<DatabaseObject>) ((ArrayList<?>) availableRooms);
