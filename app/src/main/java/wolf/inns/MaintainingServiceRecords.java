...
public class MaintainingServiceRecords {
   ...
   */
  public static ServiceRecords createServiceRecord(
      int stayId, String serviceCode, int staffId, String serviceDate, String serviceTime) {
    String sqlStatement =
        "INSERT INTO service_records(stayId, serviceCode, staffId, serviceDate, serviceTime) VALUES (?, ?, ?, ?, ?);";
   ...
   *
   * @return An ArrayList containing ServiceRecords objects for each of the returned records if
   *     successful, else null
   */
  public static ArrayList<ServiceRecords> retrieveServiceRecordsForStay(int stayId) {
    String sqlStatement =
        "SELECT stayId, serviceCode, staffId, serviceDate, serviceTime FROM service_records WHERE stayId=?;";
    Connection connection = null;
    PreparedStatement statement = null;
    ...
    try {
      ...
      // System.out.println("A list of existing service records for stayId = " +
      // stayId + " was
   ...
   *
   * @param serviceRecords the ServiceRecords object with new field values that needs to be updated
   *     in the database
   * @return true if successful, else false
   */
  public static boolean updateServiceRecord(ServiceRecords serviceRecords) {
    // Assume that we are only able to update service record when the stay is still
    // active.
    String sqlStatement =
        "UPDATE service_records SET serviceCode=?, staffId=? WHERE stayId=? AND serviceDate=? AND serviceTime=? AND stayId IN (SELECT stayId FROM stays WHERE checkoutDate IS NULL AND checkoutTime IS NULL);";
    Connection connection = null;
    PreparedStatement statement = null;
    try {
      connection = DatabaseConnection.getConnection();
      ...
      if (1 == rowsAffected) {
        ...
        System.out.println(
            "There is no corresponding existing service record or the customer stay is not active any more.");
        return false;
      } else {
        // Throw exception
        throw new SQLException("Error seems to have occured. Check the logs.");
   ...
   */
  public static ServiceRecords createCheckinCheckoutRecord(
      int stayId, int staffId, boolean checkinFlag, Connection connection) {
    String sqlStatement =
        "INSERT INTO service_records(stayId, serviceCode, staffId, serviceDate, serviceTime) VALUES (?, ?, ?, ?, ?);";
    PreparedStatement statement = null;
