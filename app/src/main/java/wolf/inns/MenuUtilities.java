...
public class MenuUtilities {
  ...
  public static DatabaseObject paginatedRecordSelection(ArrayList<DatabaseObject> recordList) {
    ...
    if (null != recordList && recordList.size() > 0) {
      ...
      if (recordListSize % PAGINATION_THRESHOLD != 0) {
        numberOfPages += 1;
      } // Add one more pages if the record list isn't divided evenly
      ...
      while (loop) {
        ...
        if (currentPageNumber < numberOfPages) {
          System.out.println(
              "Enter the record Id of the record you want to work with, press enter to go to next page, or enter 'X' to exit.");
          currentRecordId =
              easyList(
                  recordList.subList(
                      (currentRecordId - 1), (currentRecordId - 1 + PAGINATION_THRESHOLD)),
                  currentRecordId);
          System.out.print("===> ");
          ...
          System.out.println(
              "Enter the record Id of the record you want to work with, press enter to restart from page 1, or enter 'X' to exit.");
          currentRecordId =
              easyList(
                  recordList.subList((currentRecordId - 1), recordList.size()), currentRecordId);
          System.out.print("===> ");
          ...
          if (input.equals("X")) {
            ...
            if (loop) {
              // We haven't exited or chosen a record so start back from page 1 and record Id
              // 1
              currentPageNumber = 1;
              currentRecordId = 1;
            }
